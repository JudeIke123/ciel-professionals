...
if (regForm) {
  ...
  terms.addEventListener("change", () => {
    if (terms.checked) eTerms.style.display = "none";
    if (okBox) okBox.style.display = "none";
  });
  ...
  regForm.addEventListener("submit", (e) => {
    ...
    if (!fullName.value.trim() || fullName.value.trim().length < 3) {
      setError(fullName, eName, "Please enter your full name (min 3 characters).");
      ok = false;
    } else clearError(fullName, eName);

    // Email
    if (!email.value.trim() || !isEmailValid(email.value.trim())) {
      setError(email, eEmail, "Please enter a valid email address.");
      ok = false;
    } else clearError(email, eEmail);

    // Phone
    const p = digitsOnly(phone.value);
    if (!p || p.length < 10 || p.length > 14) {
      setError(phone, ePhone, "Enter a valid phone number (10–14 digits).");
      ok = false;
    } else clearError(phone, ePhone);

    // Course
    if (!course.value) {
      setError(course, eCourse, "Please select a course interest.");
      ok = false;
    } else clearError(course, eCourse);

    // Mode
    if (!mode.value) {
      setError(mode, eMode, "Please select a delivery mode.");
      ok = false;
    } else clearError(mode, eMode);

    // Password
    if (!pass.value || !strongPassword(pass.value)) {
      setError(pass, ePass, "Password must be 8+ chars with uppercase, lowercase & number.");
      ok = false;
    } else clearError(pass, ePass);

    // Confirm password
    if (!pass2.value || pass2.value !== pass.value) {
      setError(pass2, ePass2, "Passwords do not match.");
      ok = false;
    } else clearError(pass2, ePass2);

    // Terms
    if (!terms.checked) {
      eTerms.textContent = "You must agree to the Terms & Conditions.";
      eTerms.style.display = "block";
      ok = false;
    } else {
      eTerms.textContent = "";
      eTerms.style.display = "none";
    }
    ...
    // Demo success
    okBox.style.display = "block";
    okBox.textContent = "✅ Registration successful! Redirecting to login...";
